...
_JSON_SHAPE = (
    ...
    "{\n"
    '  "entries": [\n'
    '    { "detected_date": "yyyy-mm-dd or null", "text": "verbatim markdown body", "start_page": 0 }\n'
    "  ],\n"
...
_MARKDOWN_RULES = (
    ...
    "heading inside the markdown body — the date belongs only in `detected_date`."
)
...
def extract_journal_entries(
    ...
    *,
    ...
    response = client.with_options(timeout=OPENAI_JOURNAL_VISION_TIMEOUT_SECONDS).responses.create(
        model=used_model,
    ...
    for item in raw_entries:
        ...
        entries.append(
            JournalDayExtract(
                detected_date=(item.get("detected_date") or None),
                text=text,
    ...
    print(f"[scan] {len(entries)} entries — " +
          ", ".join(f"p{e.start_page}:{e.detected_date or 'no-date'}:{len(e.text)}c" for e in entries))
